...
func (p *SaveLocalState) HandleAnnounce(msg StructSaveAnnounce) error {
	...
	case Consensus:
		...
		p.ParametersCBF = castParametersCBFProva(msg.SaveAnnounce.ParametersCBF)
		if !p.IsLeaf() {
...
func (p *SaveLocalState) GetLocalData() (*html.Node, map[string]map[kyber.Point][]byte, error) {
	fmt.Printf("URL: %v\n", p.Url)
	// get data
	resp, realUrl, _, err := getRemoteDataProva(p.Url)
	if err != nil {
...
// getRemoteDataProva take a url and return: - the http response corresponding to
...
func getRemoteDataProva(url string) (*http.Response, string, *urlpkg.URL, error) {
...
// castParametersCBFProva from uint64 to uint, since uint64 is needed to send the
// paramters across the conodes
func castParametersCBFProva(param []uint64) []uint {
